    ...
    */
    private func setCallbackFunc(callback:(()->())!){
        ...
        if let cb = callback {
            ...
            self.callback = cb
            tapGesture = UITapGestureRecognizer(target: MILAlertViewManager.sharedInstance, action: #selector(UIWebView.reload)) //reload then hide if callback
            self.reloadButton.addTarget(MILAlertViewManager.sharedInstance, action: #selector(UIWebView.reload), forControlEvents: UIControlEvents.TouchUpInside)
        }else{
